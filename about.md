...
Because of our ability to closely model the spread of the virus, we are able to generate varoius hypotheticals, such as what happens if everyone began social distancing [one week earlier](/us-1weekearlier) or [one week later](/us-1weeklater). We also have hypotheticals on what the most likely outcome will be if there are [no reopenings](/us-noreopen). A model that simply uses a curve fitting function or tries to track the growth rate will not be able to generate such hypotheticals.
