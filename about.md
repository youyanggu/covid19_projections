...
We are currently making projections for: the United States, all 50 US states (plus DC, PR, VI, Guam) and 70 countries (including all 27 EU countries). Combined, these 71 countries account for 99% of all global COVID-19 deaths.
...
Since June 1, 2020, we use a variable IFR that decreases over time to reflect improving treatments and the lower age distribution of cases. Hence, we decrease the initial IFR linearly over the span of 3 months until it is 30% of the original IFR. The initial IFR is determined by the case fatality ratio, and ranges from 0.5%-1.5%. By August 2020, we estimate the IFR to be 0.2-0.4% in most of the US and Europe. This is [corroborated by CDC's best estimate scenario](https://www.cdc.gov/coronavirus/2019-ncov/hcp/planning-scenarios.html). For later-impacted regions like Latin America, we wait an additional 3 months before beginning to decrease the IFR.
